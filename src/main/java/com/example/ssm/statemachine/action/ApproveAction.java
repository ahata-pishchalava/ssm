...
public class ApproveAction implements Action<EmployeeState, EmployeeEvent> {
    private static final Logger LOGGER = LoggerFactory.getLogger(ApproveAction.class);

    @Override
    public void execute(final StateContext<EmployeeState, EmployeeEvent> context) {
        LOGGER.info("Approving Action was called for Employee with id: {}", context.getMessageHeaders().get(Employee.EMPLOYEE_ID_KEY, UUID.class));
    }
}
